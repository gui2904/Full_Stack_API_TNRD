...
const UserInterface: React.FC<UserInterfaceProps> = ({ backendName }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setUsers(response.data.reverse()); // set users in reverse mode
      } catch (error) {
  ...
  return (
    <div className={`user-interface ${bgColor} ${backendName} w-full max-w-md p-4 my-4 rounded shadow`}>
      ...
      <h2 className="text-xl font-bold text-center text-black mb-6">{`${backendName.charAt(0).toUpperCase() + backendName.slice(1)} Backend`}</h2>
